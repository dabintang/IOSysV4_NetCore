    ...
    public class EmailHelper
    {
        ...
        public static bool Send(string subject, string body, bool isBodyHtml, List<string> lstTo, List<string> lstCC, params string[] attachments)
        {
            try
            {
                ...
                //是谁发送的邮件 
                mail.From = new MailAddress(IOSysJson.Inst.AppConfig.Email.FromEmail, IOSysJson.Inst.AppConfig.Email.FromEmailName);
            ...
            {
                LogHelper.Error(ex, $"发送邮件[{subject}]失败");
                return false;
            }
        }
        ...
        {
            //邮件服务器和端口 
            var smtp = new SmtpClient(IOSysJson.Inst.AppConfig.Email.SmtpHost, IOSysJson.Inst.AppConfig.Email.SmtpPort);
            //smtp.UseDefaultCredentials = true;
            smtp.EnableSsl = true;
            //指定发送方式 
            smtp.DeliveryMethod = SmtpDeliveryMethod.Network;
            //指定登录名和密码 
            smtp.Credentials = new System.Net.NetworkCredential(IOSysJson.Inst.AppConfig.Email.FromEmail, IOSysJson.Inst.AppConfig.Email.SmtpPwd);
            //超时时间 
            smtp.Timeout = 30000;
